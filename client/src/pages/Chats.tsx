                ...
                ${chosenService ? 'translate-x-0' : 'translate-x-full'}`}>
                                    ...
                                    {c.replyTo === null ? (
                                        <motion.div
                                            ...
                                            onDragEnd={(_, info) => {
                                                ...
                                                if (info.offset.x < -60 && c.userId === userId.id) setReadStatusChat(c)
                                            }}
                                            className={`relative ${c.userId === userId.id ? "self-end bg-sky-700" : "self-start bg-zinc-800"} max-w-3/4 px-1.5 py-1 
                                            gap-2.5 items-end rounded-lg ${highlightedId === c.id ? 'ring-2 ring-amber-400 transition-all' : ''}`}
                                            ref={(el) => { messageRefs.current[c.id] = el as HTMLDivElement | null }}
                                        >
                                            <span className={`${c.userId === userId.id ? "hidden" : ""}
                                                text-[12px] text-rose-400 font-semibold`}>{c.userName}
                                            </span>
                                            <div className="text-sm wrap-break-word whitespace-pre-wrap select-text">
                                                {c.file ? (
                                                    <div
                                                        className={`${c.userId === userId.id ? "bg-sky-900" : "bg-black/25"} rounded px-1.5 py-2.5 cursor-pointer select-none`}
                                                ...
                                                <span className="invisible inline-flex gap-1 text-[10px] ml-2.5 whitespace-nowrap">
                                                    {format(new Date(c.createdAt), 'HH:mm')} {c.userId === userId.id && <CheckCheck size={13} className="text-zinc-100" />}
                                                </span>
                                            </div>
                                            <span className="absolute bottom-1 inline-flex gap-1 items-center right-1.5 text-[10px] text-zinc-300 whitespace-nowrap select-none">
                                                {format(new Date(c.createdAt), 'HH:mm')} {c.userId === userId.id &&
                                        ...
                                        <motion.div
                                            ...
                                            ref={(el) => { messageRefs.current[c.id] = el as HTMLDivElement | null }}
                                        >
                                            <span className={`${c.userId === userId.id ? "hidden" : ""} text-[12px] text-rose-400 font-semibold`}>
                                                {c.userName}
                                            </span>
                                            <div
                                                className={`${c.userId === userId.id ? "bg-sky-900" : "bg-black/25"} flex flex-col rounded-lg max-h-22.5 p-2 text-sm
                                            ...
                                            <div className="text-sm wrap-break-word whitespace-pre-wrap select-text">
                                                {c.file ? (
                                                    <div
                                                        className={`${c.userId === userId.id ? "bg-sky-900" : "bg-black/25"} rounded px-1.5 py-2.5 cursor-pointer select-none mt-1.5`}
                                                        onClick={() => window.open(c.file?.url, '_blank', 'noopener,noreffer')}
                                                    >
                                                        {c.message}
                                                    </div>
                    ...
                    {message.replyTo ? (
                        <div className="p-2 pb-0 bg-slate-700">
                            <div className={`${message.replyTo.userId === userId.id ? "bg-sky-900" : "bg-black/50"} flex flex-col rounded-lg p-2 text-sm
